...
public class FleetServiceTest {
    ...
    @BeforeEach
    public void setup(){
        when(fleetService.getFleet(existingCompany)).thenReturn(createVehicleRepresentation(existingCompany));
        when(fleetService.getFleet(nonExistingCompany)).thenReturn(new ArrayList<>());
    }

    @Test
    public void getExistingCompanyVehicles(){
        assertEquals(fleetService.getFleet(existingCompany).get(0).id,createVehicleRepresentation(existingCompany).get(0).id);
    }

    @Test
    public void getNonExistingCompanyVehicles(){
        assertEquals(fleetService.getFleet(nonExistingCompany),new ArrayList<>());
    }
