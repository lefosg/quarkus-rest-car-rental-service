...
public class ChargingPolicyResource {
    ...
    @Path("{policyId: [0-9]+}")
    @Transactional
    public ChargingPolicyRepresentation listVehicleById(@PathParam("policyId") String policyId) {
        ChargingPolicy policy = policyRepository.findById((long) Integer.parseInt(policyId));
        if (policy == null) {
    ...
    public Response create(ChargingPolicyRepresentation representation) {
        if (representation.id == null || policyRepository.findById(Long.valueOf(representation.id)) != null) {  //if id is null or already exists
